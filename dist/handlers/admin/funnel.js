...
export function createFunnelHandler() {
    async function GET(request) {
        ...
        if (!config.funnel?.stages?.length) {
            return NextResponse.json({ error: "Funnel not configured" }, { status: 404 });
        }
        ...
        const stageQueries = config.funnel.stages.map((stage) => {
            if (stage.query === "sessions") {
                ...
                return db
                    ...
                    .where(and(evDateFilter, eq(analyticsEvents.eventName, stage.query.event)));
            }
            else {
        ...
        let lastStageCountBySource = new Map();
        let lastStageCountByDevice = new Map();
        let lastStageCountByKeyword = new Map();
