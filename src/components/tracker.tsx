...
function getSessionId(prefix: string) {
...
function getGclid(
  ...
  for (const param of ["gclid", "gbraid", "wbraid"]) {
    ...
    if (val) return { value: val, type: param };
  }
...
function incrementPageViewCount(prefix: string): number {
...
function ensureSessionStart(prefix: string): number {
...
function checkReturnVisitor(prefix: string): boolean {
...
function persistAttribution(
  ...
  if (existing) {
    ...
    if (!hasNewSource || hadSource) return;
  }
  ...
  data.pageViewCount = String(
    parseInt(sessionStorage.getItem(`${prefix}-pageviews`) || "0", 10)
  );
...
export function PageTracker({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (gclidInfo) payload.gclid = gclidInfo.value;
